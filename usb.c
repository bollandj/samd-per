...
void usb_init(uint8_t gclk_48m_id, bool (*tusb_init_p)(void), void (*tud_task_p)(void), void (*tud_int_handler_p)(uint8_t))
{
	...
	gpio_pin_set_output(USB_DM_GROUP, USB_DM_PIN);
	gpio_pin_set_output(USB_DP_GROUP, USB_DP_PIN);

	gpio_pin_set_pmux(USB_DM_GROUP, USB_DM_PIN, USB_DM_PMUX_Val);
	gpio_pin_set_pmux(USB_DP_GROUP, USB_DP_PIN, USB_DP_PMUX_Val);
