...
void clock_1m_init()
{
	/* Set CLOCK_GCLK_1M clock source to 8MHz internal oscillator, /8 prescaler (1MHz) */
	GCLK->GENDIV.reg = GCLK_GENDIV_ID(CLOCK_GCLK_1M) | GCLK_GENDIV_DIV(8);
	GCLK->GENCTRL.reg = GCLK_GENCTRL_ID(CLOCK_GCLK_1M) | GCLK_GENCTRL_SRC_OSC8M | GCLK_GENCTRL_GENEN;
...
void clock_48m_init()
{
	...
	GCLK->GENDIV.reg = GCLK_GENDIV_ID(CLOCK_GCLK_48M) | GCLK_GENDIV_DIV(1);

	/* Switch GCLK to the DFLL */
	GCLK->GENCTRL.reg =
		GCLK_GENCTRL_ID(CLOCK_GCLK_48M) | GCLK_GENCTRL_IDC | GCLK_GENCTRL_GENEN | GCLK_GENCTRL_SRC_DFLL48M;
	while (GCLK->STATUS.bit.SYNCBUSY);
}

void clock_96m_init()
{
	/* Enable CLOCK_GCLK_1M and connect it to DPLL GCLK source */
	GCLK->CLKCTRL.reg = GCLK_CLKCTRL_CLKEN |
						GCLK_CLKCTRL_GEN(CLOCK_GCLK_1M) |
	...
	/* No prescaler for CLOCK_GCLK_96M */
	GCLK->GENDIV.reg = GCLK_GENDIV_ID(CLOCK_GCLK_96M) | GCLK_GENDIV_DIV(1);

	/* Switch CLOCK_GCLK_96M to the FDPLL */
	GCLK->GENCTRL.reg =
		GCLK_GENCTRL_ID(CLOCK_GCLK_96M) | GCLK_GENCTRL_IDC | GCLK_GENCTRL_GENEN | GCLK_GENCTRL_SRC_DPLL96M;
	while (GCLK->STATUS.bit.SYNCBUSY);
}